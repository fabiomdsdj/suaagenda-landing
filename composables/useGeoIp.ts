...
// Só executa no client — no SSR retorna null silenciosamente.
// Tem dedup interno: se já detectou, retorna cache sem nova request.
//
// Uso:
//   const geo = useGeoIp()
//   await geo.detect()
...
// ip-api retorna nome do estado por extenso em pt-BR → mapeia para sigla
...
export function useGeoIp() {
  ...
  async function detect(): Promise<GeoIpResult | null> {
    ...
    // Só roda no client — IP do servidor não tem valor pro usuário
    ...
    try {
      const raw = await $fetch<{
        status:      string
        city:        string
        regionName:  string  // ex: 'São Paulo'
        region:      string  // sigla ex: 'SP'
        countryCode: string
        lat:         number
        lon:         number
      ...
      if (raw.status !== 'success') {
        throw new Error(`ip-api retornou status: ${raw.status}`)
      }

      // Preferência: lookup pelo nome completo (mais confiável para acentos)
      // Fallback: sigla da API em lowercase
  ...
  return { result, citySlug, ufSlug, pending, error, detected, detect }
}