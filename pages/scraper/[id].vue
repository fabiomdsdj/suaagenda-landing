...
<template>
  ...
  onMounted(async () => {
    ...
    try {
      ...
      // ✅ FIX: fotos vêm como ApiPhoto[] com isCover — mapeia mantendo o id pra upsert
      // Ordena: capa primeiro, depois sortOrder
  ...
  function autoSlug() {
    form.slug = form.citySlug
      ? `${makeSlug(form.name)}-${form.citySlug}`
      : makeSlug(form.name)
  }
  ...
  async function handleSubmit() {
    ...
    const payload = {
      ...
      // ✅ FIX: envia photos com id (upsert no controller), isCover como 0/1
  ...
  .section-title { font-size:10px; font-weight:700; letter-spacing:.1em; text-transform:uppercase; color:#4b5563; margin-bottom:1rem; padding-bottom:.5rem; border-bottom:0.5px solid rgba(255,255,255,.05); }
