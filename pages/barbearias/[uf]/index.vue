<template>
    <div v-if="ufData" class="text-[15px]">
      ...
      <section class="w-full py-20 px-6 md:px-16 bg-[#111]">
        <div class="max-w-6xl mx-auto">
          ...
          <h2
            class="font-black leading-none mb-10 text-white"
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            >
              <div>
                <p class="font-bold text-white group-hover:text-green-400 transition-colors">
                  {{ city.city }}
                </p>
  ...
  const cities = computed(() =>
    allCities.filter((c) => c.ufSlug === ufSlug)
  )
  
  // Dados do UF a partir da primeira cidade encontrada
    ...
    computed(() => {
      ...
      return {
        ...
        meta: [
          {
            name: 'description',
            content: `Encontre barbearias em ${ufData.value.uf} com agendamento online. ${cities.value.length} cidades disponíveis.`,
          },
          { name: 'robots', content: 'index, follow' },
        ],
        link: [
          { rel: 'canonical', href: `https://suaagenda.link/barbearias/${ufSlug}` },
        ],
        script: [
          {
